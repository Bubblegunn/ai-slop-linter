...
export const boldLabel = {
    ...
    why: "A bold label and a colon on every item turns a list into a form. It is the most recognisable shape in generated documentation, and readers skim past it.",
    example: {
        before: "- **Fast:** the parser runs in one pass.\n",
        after: "- The parser runs in one pass.\n",
    },
    ignoreWhen: "The list is a glossary or an options table, where the label is the term being defined.",
    check(doc) {
...
export const titleCase = {
    ...
    why: "Title Case on every heading is a house style almost no repository uses, and it is the default a model reaches for.",
    example: {
        before: "## Getting Started With The Parser",
        after: "## Getting started with the parser",
    },
    ignoreWhen: "The project's style guide asks for title case; switch the rule off once in .slop.json.",
    check(doc) {
        const out = [];
        for (const m of doc.masked.matchAll(/^#{1,6}[ \t]+([^\n]+)$/gm)) {
            ...
            if (words.every((w) => w === w.toUpperCase()))
                continue; // ALL CAPS is a different choice
...
export const emoji = {
    ...
    why: "Emoji on headings and list markers are applied uniformly as decoration rather than for meaning, and they read badly in terminals and screen readers.",
    example: {
        before: "## \ud83d\ude80 Getting started",
        after: "## Getting started",
    },
    ignoreWhen: "The emoji is the content, such as documentation of which emoji a command prints.",
    check(doc) {
...
export const curlyQuotes = {
    ...
    why: "Curly marks arrive when text is pasted out of an interface that substitutes them. The rule fires only on a file that mixes them with straight marks, because a file that uses them throughout made a typographic choice.",
    example: {
    ...
    ignoreWhen: "The file is typeset prose that uses curly marks everywhere, in which case the rule already says nothing.",
    check(doc) {
        ...
        const findings = [];
        if (/"/.test(doc.masked))
            findings.push(...scan(doc, curlyQuotes, /[\u201C\u201D]/g, () => "curly quote among straight ones; the file mixes both", () => '"'));
        if (/'/.test(doc.masked))
            findings.push(...scan(doc, curlyQuotes, /[\u2018\u2019]/g, () => "curly apostrophe among straight ones; the file mixes both", () => "'"));
        return findings.sort((a, b) => a.line - b.line || a.column - b.column);
    },
};
/** Too many hyphenated word pairs per hundred words. */
export const hyphenDensity = {
    id: "hyphen-density",
    title: "Hyphenated compounds everywhere",
    severity: "info",
    source: "Wikipedia, Signs of AI writing: Overuse of hyphenated compounds",
    why: "A high rate of hyphenated compounds is the signature of a model reaching for adjectives. Most of them lose the hyphen when they follow the noun.",
    example: {
    ...
    ignoreWhen: "The compounds are established technical terms in your domain and dropping the hyphen would change the meaning.",
    check(doc) {
